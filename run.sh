	...
	do
		...
		do
			...
			do
				...
				model="heur"
				for init_solve in 1 2 3
				do
					for subsampling in 100 
					do
						ALGO=${model}_i${init_solve}_s${subsampling}
						nohup julia main.jl $DIR$INS$ALGO $id $seed $num_obs $noise_level $TIMELIMIT $model $init_solve $subsampling > $DIR$INS$ALGO.console 2>&1 &
						cnt=$(( cnt + 1 ))
						continue
					done
				done
